...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Define the upload directory and public image directory
    $uploadDir = "/var/www/uploads/";
    $publicImageDir = "/uploads/";

    $imagePath = $publicImageDir . basename($_FILES["image"]["name"]);
    $publicImagePath = $publicImageDir . basename($_FILES["image"]["name"]);

    if (move_uploaded_file($_FILES["image"]["tmp_name"], $imagePath)) {
        // Insert data into the database with the public image path
        $sql = "INSERT INTO user_pages (title, content, image_path) VALUES (?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sss", $title, $content, $publicImagePath);
